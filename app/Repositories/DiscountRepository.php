...
{
    public function __construct(Discount $modelo)
    {
            ...
            $query = QueryBuilder::for($this->model->query())
                ...
                ->allowedSorts([
                    ...
                    if (! empty($request['course_id'])) {
                        $courseDay = CourseDay::where('course_id', $request['course_id'])->pluck('id');
                        $query->whereIn('day_course_id', $courseDay);
                    }
        ...
        $data = $this->model->select($select)
            ...
            ->where(function ($query) use ($request, $idsAllowed) {
                if (! empty($request['day'])) {
                    $query->where('id', 'like', '%'.$request['day'].'%');
                }
            ...
            ->where(function ($query) use ($request) {
                if (! empty($request['searchQueryInfinite'])) {
                    $query->orWhere('day', 'like', '%'.$request['searchQueryInfinite'].'%');
                }
            });
