...
{
    public function __construct(Level $modelo)
    {
        ...
        $data = $this->model->select($select)
            ...
            ->where(function ($query) use ($request, $idsAllowed) {
                if (! empty($request['level'])) {
                    $query->where('id', 'like', '%'.$request['level'].'%');
                }
            ...
            ->where(function ($query) use ($request) {
                if (! empty($request['searchQueryInfinite'])) {
                    $query->orWhere('level', 'like', '%'.$request['searchQueryInfinite'].'%');
                }
            });
