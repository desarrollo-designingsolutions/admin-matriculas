...
{
    public function __construct(Period $modelo)
    {
        ...
        $data = $this->model->select($select)
            ...
            ->where(function ($query) use ($request, $idsAllowed) {
                if (! empty($request['period'])) {
                    $query->where('id', 'like', '%'.$request['period'].'%');
                }
            ...
            ->where(function ($query) use ($request) {
                if (! empty($request['searchQueryInfinite'])) {
                    $query->orWhere('period', 'like', '%'.$request['searchQueryInfinite'].'%');
                }
            });
